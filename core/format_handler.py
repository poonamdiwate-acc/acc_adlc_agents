...
Rules:
1. Extract ALL requirements you can identify from the document — functional, \
non-functional, integration, and business rules.
2. PRESERVE the original requirement IDs exactly as written in the document \
(e.g. NFR-UC2-001, FR-UC2-003, IR-KAFKA-001, NFR-001, FR-001, BR-001). \
Do NOT rename or reassign IDs.
3. If a requirement truly has no explicit ID in the document, assign one as \
REQ-001, REQ-002, etc. But if the document shows IDs like NFR-001, FR-001, \
IR-001, use those EXACTLY.
4. Set "type" based on the requirement's original category: \
"non_functional" for NFR-prefixed items, "functional" for FR-prefixed, \
"integration" for IR-prefixed, "business_rule" for BR-prefixed.
5. If priority is not stated, infer from context or default to "medium".
6. If acceptance_criterion is not stated, set to empty string.
7. When a document contains multiple requirement tables (Functional, \
Non-Functional, Integration, Business Rules), extract ALL tables — \
not just one.
...
_DE06_EXTRACTION_PROMPT = """\
You are a document extraction assistant. Your job is to extract structured \
data from free-form documents and return it as a JSON object.

For non-functional design input, extract the following:

- "structured_requirements": SINGLE array of ALL requirement objects found \
in the document. Each object has {req_id, title, description, type, \
priority, acceptance_criterion, source_ref}. Include functional, \
non-functional, integration, and business rule requirements — the \
downstream agent will filter what it needs.

- "agent_network_html": string — if the document contains architecture \
descriptions, system topology, service diagrams, agent networks, or \
infrastructure descriptions, concatenate them into this field as a string. \
Skip if not present.

Rules:
1. Extract ALL requirements you can identify from the document — functional, \
non-functional, integration, and business rules. The downstream agent needs \
NFR items but uses ALL items for cross-referencing.
2. PRESERVE the original requirement IDs exactly as written in the document \
(e.g. NFR-UC2-001, FR-UC2-003, IR-KAFKA-001, NFR-001, FR-001, BR-001). \
Do NOT rename or reassign IDs.
3. If a requirement truly has no explicit ID in the document, assign one as \
...
5. If priority is not stated, infer from context or default to "medium".
6. If acceptance_criterion is not stated, set to empty string.
7. When a document contains multiple requirement tables or sections, \
extract ALL of them into the single "structured_requirements" array.
8. Return ONLY the JSON object. No explanation, no markdown, no preamble.
"""
